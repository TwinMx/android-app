...
public class TMBluetoothService {
    private static final String TAG = "TMBluetoothService";
    ...
    private final TMBluetoothDataManager mDataManager;
    ...
    public TMBluetoothService(Handler handler, TMBluetoothDataManager dataManager) {
        mAdapter = BluetoothAdapter.getDefaultAdapter();
        mState = TMBluetoothService.STATE_NONE;
        mHandler = handler;
    ...
    private synchronized void setState(int state) {
        ...
        mHandler.obtainMessage(TMBluetoothService.MESSAGE_STATE_CHANGE, state, -1).sendToTarget();
    }
    ...
    public synchronized void start(boolean android, boolean secure) {
        ...
        setState(TMBluetoothService.STATE_LISTEN);

        // Start the thread to listen on a BluetoothServerSocket
        if (mSecureAcceptThread == null) {
            mSecureAcceptThread = new TMBluetoothService.AcceptThread(isAndroid, mIsSecure);
    ...
    public synchronized void connect(BluetoothDevice device) {
        // Cancel any thread attempting to make a connection
        if (mState == TMBluetoothService.STATE_CONNECTING) {
            if (mConnectThread != null) {mConnectThread.cancel(); mConnectThread = null;}
        }
        ...
        mConnectThread = new TMBluetoothService.ConnectThread(device);
        mConnectThread.start();
        setState(TMBluetoothService.STATE_CONNECTING);
    }
    ...
    public synchronized void connected(BluetoothSocket socket, BluetoothDevice
        ...
        mConnectedThread = new TMBluetoothService.ConnectedThread(socket, socketType);
        ...
        setState(TMBluetoothService.STATE_CONNECTED);
    }
    ...
    public synchronized void stop() {
        ...
        setState(TMBluetoothService.STATE_NONE);
    }
    ...
    public void write(byte[] out) {
        ...
        TMBluetoothService.ConnectedThread r;
        // Synchronize a copy of the ConnectedThread
        synchronized (this) {
            if (mState != TMBluetoothService.STATE_CONNECTED) return;
            r = mConnectedThread;
        }
    ...
    private void connectionFailed() {
        ...
        TMBluetoothService.this.start(isAndroid, mIsSecure);
    }
    ...
    private class AcceptThread extends Thread {
        ...
        public void run() {
            ...
            while (mState != TMBluetoothService.STATE_CONNECTED && isRunning) {
                try {
                ...
                if (socket != null) {
                    synchronized (TMBluetoothService.this) {
                        switch (mState) {
                            case TMBluetoothService.STATE_LISTEN:
                            case TMBluetoothService.STATE_CONNECTING:
                                // Situation normal. Start the connected thread.
                                connected(socket, socket.getRemoteDevice(),
                                        mSocketType);
                                break;
                            case TMBluetoothService.STATE_NONE:
                            case TMBluetoothService.STATE_CONNECTED:
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            while (true) {
                try {
                    ...
                    //mHandler.obtainMessage(TMBluetoothService.MESSAGE_READ, data).sendToTarget(); //TODO : Remove handler
                    mDataManager.addFrame(data);
                } catch (IOException e) {
                    connectionLost();
                    // Start the service over to restart listening mode
                    TMBluetoothService.this.start(isAndroid, mIsSecure);
        ...
        public void write(byte[] buffer) {
            try {
                ...
                mHandler.obtainMessage(TMBluetoothService.MESSAGE_WRITE
